  ...
  async generateVoiceReply(
    ...
    if (!this.callHistories.has(sessionId)) {
      ...
      const apptRule = !dentallyOn ? '' : (isEs
        ? '\n\nPROTOCOLO DE CITAS: Antes de consultar disponibilidad SIEMPRE pregunta al paciente (1) qué día concreto desea y (2) si tiene preferencia de profesional. NUNCA asumas la fecha de hoy ni elijas un profesional por tu cuenta. Llama a la herramienta dentally_check_availability SOLO después de que el paciente te diga una fecha concreta; pásale esa fecha exacta y, si la dio, el nombre del profesional. Si el paciente no tiene preferencia de profesional, ofrécele listarlos con dentally_list_practitioners. Lee únicamente los horarios reales que te devuelva la herramienta; nunca inventes horarios ni digas que no hay disponibilidad sin haber llamado a la herramienta con la fecha que pidió el paciente.'
        : '\n\nAPPOINTMENT PROTOCOL: Before checking availability ALWAYS ask the patient (1) which exact day they want and (2) whether they prefer a specific practitioner. NEVER assume today\'s date or pick a practitioner on your own. Call the dentally_check_availability tool ONLY after the patient gives you a concrete date; pass that exact date and, if given, the practitioner name. If the patient has no preference, offer to list practitioners with dentally_list_practitioners. Read only the real times the tool returns; never invent times or say there is no availability without having called the tool with the date the patient requested.');
      this.callHistories.set(sessionId, [{ role: 'system', content: (bot.system_prompt ?? '') + dateRule + apptRule + hangupRule }]);
