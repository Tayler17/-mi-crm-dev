...
function buildTools(stageNames: string[], tagNames: string[] = [], dentallyConnected = false): Array<{ name: string; description: string; parameters: any }> {
  ...
  if (dentallyConnected) {
    tools.push(
      ...
      { name: 'dentally_book_appointment', description: "Book an appointment as soon as the caller picks a day and time from the list — call this, do NOT re-check availability for the same date. ALWAYS pass the patient's own name (and phone if they gave one) so the clinic registers the right person — do NOT rely on the caller's phone number, which may differ. If the caller says they are ALREADY a patient, pass their name (and phone if given); only if they are NOT a patient, also ask for date_of_birth (YYYY-MM-DD) and gender (male/female), then call this.", parameters: { type: 'object', properties: { date: { type: 'string', description: 'YYYY-MM-DD' }, time: { type: 'string', description: 'HH:MM 24h' }, name: { type: 'string', description: "Patient's full name as given by the caller" }, phone: { type: 'string', description: "Patient's phone number as given by the caller (may differ from the caller's number)" }, email: { type: 'string', description: "Patient's email if given" }, practitioner_name: { type: 'string' }, duration: { type: 'number' }, reason: { type: 'string' }, date_of_birth: { type: 'string' }, gender: { type: 'string', enum: ['male', 'female'] }, title: { type: 'string' } }, required: ['date', 'time'] } },
    );
  }
  return tools;
}
...
- update_deal: para actualizar un trato existente.${crmCtx.stages.length ? `\nEtapas de pipeline: ${crmCtx.stages.map((s: any) => s.pipeline_name ? `${s.name} (${s.pipeline_name})` : s.name).join(', ')}.` : ''}`;
  ...
  async buildVoiceAgentSettings(bot: any, callSid: string): Promise<any> {
    ...
    const apptRule = !dentallyConnected ? '' : (isEs
      ? 'CITAS: Para disponibilidad lo único obligatorio es la FECHA; si no la dieron, pregúntala (nunca asumas hoy). El profesional es OPCIONAL. Pregunta SIEMPRE el NOMBRE del paciente y pásalo en dentally_book_appointment (más el teléfono si lo dan); NO identifiques al paciente por el número desde el que llaman, puede ser distinto. En cuanto elijan horario, agenda con dentally_book_appointment. Para un paciente NUEVO pide además fecha de nacimiento y género; para uno existente basta el nombre.'
      : 'APPOINTMENTS: The only required field for availability is the DATE; if not given, ask for it (never assume today). The practitioner is OPTIONAL. ALWAYS ask for the PATIENT\'S NAME and pass it in dentally_book_appointment (plus phone if given); do NOT identify the patient by the number they are calling from, it may be different. As soon as they pick a time, book with dentally_book_appointment. For a NEW patient also ask for date of birth and gender; for an existing one the name is enough.');
