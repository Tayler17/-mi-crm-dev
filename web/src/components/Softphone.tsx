...
export function Softphone() {
  ...
  // Draggable position (remembered): the user can drag the softphone anywhere.
  useEffect(() => { try { const s = localStorage.getItem('softphonePos'); if (s) setPos(JSON.parse(s)); } catch {} }, []);
  useEffect(() => { if (pos) try { localStorage.setItem('softphonePos', JSON.stringify(pos)); } catch {} }, [pos]);

  const dragHandlers = {
    onPointerDown: (e: React.PointerEvent) => {
      const el = (e.currentTarget as HTMLElement).closest('[data-softphone]') as HTMLElement | null;
    ...
    onPointerMove: (e: React.PointerEvent) => {
      ...
      if (d.moved) {
        const left = Math.max(4, Math.min(window.innerWidth - 56, d.ox + dx));
        const top = Math.max(4, Math.min(window.innerHeight - 56, d.oy + dy));
        setPos({ left, top });
      }
    },
  ...
  // Anchor: dragged position if set, else bottom-left on desktop / top-left on mobile.
  const anchor: React.CSSProperties = pos
    ? { left: pos.left, top: pos.top }
    : (isMobile ? { top: 64, left: 12 } : { bottom: 24, left: 24 });
  ...
  if (status === 'ready') {
    return (
      <button
        data-softphone
        {...dragHandlers}
        onClick={() => { if (didDragRef.current) { didDragRef.current = false; return; } setDialInput(''); setStatus('dialer'); }}
        title="Marcar un número (arrástralo para moverlo)"
        style={{ position: 'fixed', ...anchor, zIndex: 900, background: 'var(--surface, var(--bg))', border: '1px solid var(--border)', borderRadius: 20, padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 6, boxShadow: '0 4px 14px rgba(0,0,0,0.15)', fontSize: 12, color: 'var(--text-muted)', cursor: 'grab', touchAction: 'none' }}
      >
  ...
  if (status === 'dialer') {
    ...
    return (
      <div data-softphone style={{ position: 'fixed', ...anchor, zIndex: 950, width: 300, maxWidth: 'calc(100vw - 32px)', background: 'var(--surface, var(--bg))', border: '1px solid var(--border)', borderRadius: 16, boxShadow: '0 16px 48px rgba(0,0,0,0.32)', overflow: 'hidden' }}>
        <div {...dragHandlers} style={{ padding: '12px 16px', background: 'linear-gradient(135deg,#4f46e5,#6366f1)', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', touchAction: 'none', cursor: 'grab' }}>
  ...
  if (status === 'warm') {
    return (
      <div data-softphone style={{ position: 'fixed', ...anchor, zIndex: 950, width: 300, maxWidth: 'calc(100vw - 32px)', background: 'var(--surface, var(--bg))', border: '1px solid var(--border)', borderRadius: 16, boxShadow: '0 16px 48px rgba(0,0,0,0.32)', overflow: 'hidden' }}>
        <div {...dragHandlers} style={{ padding: '14px 18px', background: 'linear-gradient(135deg,#7c3aed,#a855f7)', color: '#fff', touchAction: 'none', cursor: 'grab' }}>
  ...
  return (
    <div style={{ position: 'fixed', ...anchor, zIndex: 950, width: 300, maxWidth: 'calc(100vw - 32px)', background: 'var(--surface, var(--bg))', border: '1px solid var(--border)', borderRadius: 16, boxShadow: '0 16px 48px rgba(0,0,0,0.32)', overflow: 'hidden' }}>
      <div {...dragHandlers} style={{ padding: '14px 18px', background: incoming ? 'linear-gradient(135deg,#16a34a,#22c55e)' : 'linear-gradient(135deg,#4f46e5,#6366f1)', color: '#fff', touchAction: 'none', cursor: 'grab' }}>
